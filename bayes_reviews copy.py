...
# print(word_prob)
vocab = len(word_count.keys())
total_spam_words = sum(word_count_target['spam'].values())
total_ham_words = total_words-total_spam_words
word_target_prob = {'spam':{key:(value+1)/(total_spam_words+vocab) for key, value in word_count_target['spam'].items()}, \
                        'ham':{key:(value+1)/(total_ham_words+vocab) for key,value in word_count_target['ham'].items()}}
